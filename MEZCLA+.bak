...
;            := vector [<expresion> <identificador>]
;               vector-exp (exp1 id)

;            := vector-ref [<identificador> <expresion>]
;               vector-ref-exp (id exp2)

;            := vector-set [<identificador> <expresion> <expresion>]
;               vector-set-exp (id exp2 exp3)    
...
;            := for (<expresion(list-exp)>){<expresion(procedimiento-exp)>}
;               for-exp(ls proc)
...
(define gramatica
  '(
    ...
    (expresion ("Var" id "=" expresion) mutable-exp)
    (expresion ("begin" "{"expresion (arbno ";" expresion) "}") begin-exp)
    ...
    ;;Vectores
    (expresion ("vector" "[" num id "]") vector-exp)
    (expresion ("vector-ref" "[" id num "]") vector-ref-exp)
    (expresion ("vector-set" "[" id num expresion "]") vector-set-exp)
    ;; booleanos
    ...
    ;;vectores
    (primitiva-unaria ("vector?") primitiva-pred-vector)
    ;;string
    (primitiva-unaria ("longitud") primitiva-longitud)
    ;;base10
    ...
    (primitiva-unaria ("!")primitiva-not) ;; not
      ...
      (expresion ("for" "(" expresion ")" "{" expresion ";" "}" ) for-exp )
...
(define eval-exp
  (lambda (expre env)
    (cases expresion expre
      ...
      (mutable-exp (id valor-exp) (mutable-dcl id (eval-exp valor-exp env)))
      ...
      (begin-exp (exp exps) 
                 (let loop ((acc (eval-exp exp env))
                             (exps exps))
                    (if (null? exps) 
                        acc
                        (loop (eval-exp (car exps) 
                                               env)
                              (cdr exps)))))
      ...
      (lista-vacia-exp () '())
      (vector-exp (num id) (make-vector num id))
      (vector-ref-exp (id num) (vector-ref (buscar-variable env id) num))
      (vector-set-exp (id num exp) (vector-set! (buscar-variable env id) num (eval-exp exp env)))
      (for-exp (ls proc) (printFor (eval-exp ls env) proc env))
      (else expre)
      )
    )
  )
...
(define aplicar-un-exp
  (lambda (primitiva exp env)
    (cases primitiva-unaria primitiva
      ...
      (primitiva-pred-vector () (vector? (eval-exp exp env)))
    ...
    (cases reference (apply-env-ref env id)
      (a-ref (pos vals states)
             (cond
               ...
               [(eqv? (vector-ref states pos) 'mutable)(begin(setref!(apply-env-ref env id) valor 'mutable)(buscar-variable env id))]
               )
             )
      )
    )
...
(define-datatype declaracion declaracion?
  ...
  (mutable-dcl (id symbol?) (valor valor-racket?))
  )
  ...
  (lambda (decl)
    (if (eqv? decl empty)
        ...
        (cases declaracion (car decl)
          ...
          (mutable-dcl (id value) (cons id (dcl-ids (cdr decl))))
          )
        )
    )
  )
  ...
  (lambda (decl)
   (if (eqv? decl empty)
        ...
        (cases declaracion (car decl)
          ...
          (mutable-dcl (id value) (cons value (dcl-values (cdr decl))))
          )
        )
    )
  )
  ...
  (lambda (decl)
    (if (eqv? decl empty)
        ...
        (cases declaracion (car decl)
          ...
          (mutable-dcl (id value) (cons 'mutable (dcl-states (cdr decl))))
          )
        )
    )
  )
...
;;Funcion que recibe un procedimiento (datatype proVal) y una lista de expresiones y ejecuta el procedimiento
;;evaluando la lista de expresiones para los ids del proc en el ambiente en el que fue creada la cerradura
(define evaluar-procedimiento-for
 (lambda (proc value-exps env)
   ;proc es un Id que referencia un procedimiento en el ambiente
   (cases procVal (eval-exp proc env)
     (closure (ids exp clousured-env)(apply-clousure-exp ids value-exps exp clousured-env))
     )
   )
 )

;;Funcion que itera una lista y devuelve valores apartir de un procedimiento que se le pase
;; fXlsXenvXi -> donde el procedimiento f recibe valores de la lista ls teniendo en cuenta un ambiente env y una varianble que aumenta i
(define forLogic
  (lambda (x f env i)
    (if (null? (cdr x))
        (typeData (evaluar-procedimiento-for f (list (car x)) env) #t)
        (begin (extend-env (list (string->symbol (string-append "ite-" (number->string i)))) '((evaluar-procedimiento-for f (list (car x)) env)) (list "const") env) (string-append (typeData (evaluar-procedimiento-for f (list (car x)) env) #t) "\n" (forLogic (cdr x) f env (+ i 1)))))

    ))

;;funcion auxiliar que retorna los valores que genera un iterador
;;lsxprocXenv -> donde recibe una procedimiento proc , una lista ls y un ambiente env
(define printFor (lambda (ls proc env)
                (begin (eopl:printf "~a \n" (forLogic ls proc env 0)) "Fin del Ciclo" )
                ))

;;funcion que convierte en string dependiendo del tipo de dato que se le pasa
;;xXflag -> donde el valor que se le pasa es x y se tiene una bandera flag para validar las listas
(define typeData (lambda (x flag)
                   (cond
                     [(number? x) (number->string x)]
                     [(string? x) x]
                     [(boolean? x) (if (eqv? #t x) "true" "false") ]
                     [(list? x) (if (eqv? flag #t) (string-append "(" (number->string (car x)) " " (typeData (cdr x) #f)) (if (null? (cdr x)) (string-append (number->string (car x) ) ")") (string-append (number->string (car x)) " " (typeData (cdr x) #f)) ) ) ]
                     )
...
;; Begin
;; @x mutable
(scan&parse "begin{asignar @x = 1; ; asignar @x = 2 ; ; asignar @x = 3 ; ; @x}")

;; Set
(scan&parse "asignar @p = 8;") ; set-expresion
;; Variables mutables (@x mutable)
(scan&parse "sea(const @o = @x ; const @m = asignar @x = 1 ; ; const @n = asignar @x = 2;){ lista[@o, @m, @n, @x];}")
;; Cambio de estado en variables de asignacion unica (@y unicVal)
(scan&parse "sea(const @m = asignar @y = 1 ; ; const @n = asignar @y = 2;){ @n;}")
...
;iterador
(scan&parse "for(lista[1,2,3]){proc(@x){(@x + 1)};}")